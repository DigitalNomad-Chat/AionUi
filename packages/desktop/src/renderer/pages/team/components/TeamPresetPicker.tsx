...
import TeamPresetEmptyState from './TeamPresetEmptyState';
...
const TeamPresetPicker: React.FC<Props> = ({ presets, selectedId, onSelect, onInvoke, onCreate, onEdit, onRemove }) => {
  ...
  return (
    <div className='flex h-full flex-col' data-testid='team-preset-picker'>
      ...
      {presets.length === 0 ? (
        <TeamPresetEmptyState onCreate={onCreate} />
      ) : (
        <>
          <Input.Search
            ...
            {presets.map((preset) => (
              ...
              >
                <Button
                  ...
                  className={`absolute inset-0 z-0 h-auto !rounded-8px border px-12px py-10px ${preset.id === selectedId ? 'border-primary bg-[rgba(var(--primary-6),0.08)]' : 'border-border-2 bg-fill-1 hover:border-primary hover:bg-fill-2'}`}
                />
                <div className='relative z-10 flex w-full items-center gap-10px px-12px py-10px pointer-events-none'>
                  ...
                  <Button
                    ...
                    className='pointer-events-auto opacity-0 group-hover:opacity-100'
                    onClick={(e) => {
                      e.stopPropagation();
                      onInvoke(preset);
                    }}
