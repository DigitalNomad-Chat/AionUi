      ...
      {adHocTeam.association?.team_id && (
        <div className='shrink-0'>
          <TeamStatusCard
            ...
            onNavigate={(teamId) => {
              // Active teams navigate to the team page; a disbanded team hands
              // back its origin_conversation_id and must land on the source
              // conversation route instead of a non-existent /team/<id>.
              adHocTeam.clearUnreadTeammateMessages();
              const isDisbanded = adHocTeam.association?.status === 'disbanded';
              void navigate(isDisbanded ? `/conversation/${teamId}` : `/team/${teamId}`);
            }}
          />
        </div>
      )}
